...
export default function PaginationComments({comments}) {
    ...
    >
      <ul className="inline-flex -space-x-px text-base h-10">
        <button
          ...
          className="flex items-center justify-center px-4 h-10 ms-0 leading-tight text-gray-500 bg-white border border-r-0 border-gray-300 rounded-s-lg hover:bg-gray-100 hover:text-gray-700   cursor-pointer"
        >
          Previous
        </button>
        {Array.from({ length: Math.floor(dataLength / 2) + 1 }, (_, index) => (
          <button
            key={index + 1}
